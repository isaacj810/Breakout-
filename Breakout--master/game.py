import pygame
from elements import Paddle, Ball, Block, create_level, ASSETS_DIR
from elements import LEVEL_1, LEVEL_2, LEVEL_3, LEVEL_4, LEVEL_5
from elements import LEVEL_6, LEVEL_7, LEVEL_8, LEVEL_9, LEVEL_10
...
class Game:
    def __init__(self):
        bg = pygame.image.load(str(ASSETS_DIR / "fondo_juego.png"))
        ...
        self.font             = pygame.font.Font(str(ASSETS_DIR / "StackedStrong-Regular.otf"), 32)
        ...
        self.powerups       = []
        self.active_effects = {}
        self.effect_duration = 10_000   # 10 s en ms

        self.power_sound = pygame.mixer.Sound(str(ASSETS_DIR / "music" / "sonido_powerup.mp3"))
        self.power_sound.set_volume(0.7)
        self.hit_sound = pygame.mixer.Sound(str(ASSETS_DIR / "music" / "sonido_hit.mp3"))
    ...
    def update(self):
        ...
        for power in self.powerups[:]:
            ...
            elif power.rect.top > settings.HEIGHT:
                self.powerups.remove(power)
                if power in self.game_objects:
                    self.game_objects.remove(power)
    ...
    def restart_level(self):
        ...
        self.game_objects.extend(self.blocks)
        self.ball.reset()
        self.paddle.rect.centerx = settings.WIDTH // 2
    ...
    def activate_power(self, power_type):
        ...
        if power_type == "x2":
            pass  # Manejado en handle_collisions
            ...
            self.paddle.image = pygame.transform.scale(
                pygame.image.load(str(ASSETS_DIR / "plataforma.png")), (120, 30)
            )
        ...
        for effect in list(self.active_effects):
            if current_time - self.active_effects[effect] > self.effect_duration:
                ...
                if effect == "slow":
                    ...
                    center = self.paddle.rect.center
                    self.paddle.image = pygame.transform.scale(
                        pygame.image.load(str(ASSETS_DIR / "plataforma.png")), (180, 30)
                    )
                    self.paddle.rect = self.paddle.image.get_rect()
                    self.paddle.rect.center = center
